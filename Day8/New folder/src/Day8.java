...
public class Day8 {
	public static void main(String[] args) {
		...
		for(int i = 0; i < client.length; i++) {
			client[i] = new HospitalClient(i, names[(int)(Math.random()*names.length)], lastnames[(int) (Math.random()*lastnames.length)], surnames[(int) (Math.random()*surnames.length)], Date.UTC(1999, 12, 13, 13, 45, 15) ,adresses[(int) (Math.random()*adresses.length)], Date.UTC(1999, 12, 13, 13, 45, 15));
		}

		task1.printClient(client);
